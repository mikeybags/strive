...
class TestTask(TestCase):
    def test_model(self):
        ...
        new_task = Task.objects.create_task(user.id,"task1", description="description of task", start_date="2017-05-30", end_date="2017-05-30", points=15, task_type="regular")
    ...
    def test_model_completed(self):
        ...
        new_task = Task.objects.create_task(user.id,"task1", "description of task", start_date="2017-05-30", end_date="2017-05-30", points=15, task_type="recurring")
        task = Task.objects.get(id=1)
    ...
    def test_model_updatedtask(self):
        ...
        task = Task.objects.get(id=1)
        update_task = Task.objects.update_task(task.id, "updated_task", "description of task", "2017-06-30", "2017-06-30", points=15, task_type="medium")
...
class TestGroupMemeber(TestCase):
    ...
    def test_model_accepted(self):
        ...
        user = User.objects.get(id=1)
        user2 = User.objects.get(id=2)
        new_task = Task.objects.create_task(user.id,"task1", "description of task", start_date="2017-05-30", end_date="2017-05-30", points=15, task_type="regular")
        task = Task.objects.get(id=1)
        new_group = Group.objects.create_group("First Group", 300, task.id, "2017-04-27 16:48:00.000")
        group = Group.objects.get(id=1)
        new_group_member = GroupMember.objects.create_member(group.id, user.id)
